...
class AddToCartView(LoginRequiredMixin, DetailView, FormView):
    ...
    def form_valid(self, form):
        ...
        med_ex = Cart.objects.filter(
            medicine=ob.id, username=self.request.user.id).exists()
        ...
        user_data = Cart.objects.filter(
            username=self.request.user.id).values('medicine_id')
        ...
        if med_ex and user_ex:
            k = list(Cart.objects.filter(username=self.request.user.id,
                     medicine=ob.id).values('c_quantity', 'total_price'))
            q += k[0]['c_quantity']
            p = q * med.price
            Cart.objects.filter(username=self.request.user.id, medicine=ob.id).update(
                c_quantity=q, total_price=p)
        else:
